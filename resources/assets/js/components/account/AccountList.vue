<template>
    <div>
        <el-card class="box-card" shadow="never">
            ...
            <el-table class="yx_table" border :data="accountlist">
                ...
                <el-table-column  label="操作">
                    <template slot-scope="scope">
                        <el-tooltip class="item" effect="dark" content="编辑" placement="bottom-start">
                            <el-button type="primary" icon="el-icon-edit" @click="edit('/accountedit',scope.row.id,2)" circle></el-button>
                        </el-tooltip>
                        <el-tooltip class="item" effect="dark" content="查看" placement="bottom-start">
                            <el-button type="info" icon="el-icon-notebook-1" @click="edit('/accountedit',scope.row.id,0)" circle></el-button>
                        </el-tooltip>
    ...
    export default {
        ...
        methods: {
            getList(){
                ...
                this.$store.dispatch('loadAccountList',send_data);
            },
            add(){
                this.$router.push('/accountedit/-1/1');
            },
            ...
            del(id){
                ...
                that.$store.dispatch('delAccount',send_data);
                })
            },
            //跳转页
        ...
        computed:{
            ...
            total_count(){
                return this.$store.getters.getAccountTotalCount;
            },
            page_sizes(){
                return this.$store.state.page_sizes;
            },
            delAccountStatus(){
                return this.$store.getters.getDelAccountStatus;
            }
        },
        watch:{
            delAccountStatus(val){
                let that=this;
                if(val==1){
                    this.$message({
                        message:'删除成功',
                        ...
                        duration:1000,
                        onClose:function(e){
                            that.$store.commit('setDelAccountStatus',-1);
                            that.reload();
                        }
                    });
                }else if(val==0){
